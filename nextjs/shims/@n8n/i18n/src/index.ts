...
type GlobalI18n = {
	...
	setLocaleMessage: (language: string, messages: LocaleMessages) => void;
	setNumberFormat: (language: string, formats: unknown) => void;
	mergeLocaleMessage: (language: string, messages: LocaleMessages) => void;
};
...
const globalI18n: GlobalI18n = {
	locale: { value: 'en' },
	fallbackLocale: 'en',
	messages: { en: englishBaseText as unknown as LocaleMessages },
	numberFormats: {},
	te: function (key: string): boolean {
		const msg = resolvePath(this.messages[this.locale.value] || {}, key);
	...
	t: function (
		...
		const current = resolvePath(this.messages[lang] || {}, key);
		if (current === undefined || current === null) {
			const fallback = resolvePath(this.messages[this.fallbackLocale] || {}, key);
	...
	setLocaleMessage: function (language: string, messages: LocaleMessages) {
		this.messages[language] = messages;
	},
	setNumberFormat: function (language: string, formats: unknown) {
		this.numberFormats[language] = formats;
	},
	mergeLocaleMessage: function (language: string, messages: LocaleMessages) {
		const existing = (this.messages[language] || {}) as Record<string, unknown>;
		this.messages[language] = deepMerge(existing, messages as Record<string, unknown>);
	},
};

export const i18nInstance = { global: globalI18n };
...
export async function loadLanguage(language: string) {
	...
	const { numberFormats, ...rest } = (await import(`@n8n/i18n/locales/${language}.json`)).default as any;

	i18nInstance.global.setLocaleMessage(language, rest as unknown as LocaleMessages);
