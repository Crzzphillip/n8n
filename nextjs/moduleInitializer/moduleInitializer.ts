// Converted from Vue Router to a framework-agnostic router integration
...
import type { FrontendModuleDescription, ModuleRoute } from '@/moduleInitializer/module.types';
 ...
 */
export type AddRouteFn = (route: ModuleRoute) => void;

export const registerModuleRoutes = (addRoute: AddRouteFn) => {
	modules.forEach((module) => {
		module.routes?.forEach((route) => {
			// Prepare the enhanced route with module metadata and custom middleware that checks module availability
			const enhancedRoute: ModuleRoute = {
				...route,
				meta: {
					...route.meta,
