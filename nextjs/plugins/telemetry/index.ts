...
type SimpleRoute = {
	...
	path: string;
	meta?: { telemetry?: { pageCategory?: string; getProperties?: (route: SimpleRoute) => any } };
};

export class Telemetry {
	private pageEventQueue: Array<{ route: SimpleRoute }>;
	...
	page(route: SimpleRoute) {
		if (this.rudderStack) {
			...
			this.pageEventQueue.push({ route });
		}
	}
	...
	flushPageEvents() {
		...
		this.pageEventQueue = [];
		queue.forEach(({ route }) => this.page(route));
	}
	...
	private initRudderStack(key: string, proxy: string, options: IDataObject) {
		...
		this.rudderStack.loadJS = () => {
			...
			const element: Element | null = document.getElementsByTagName('script')[0] ?? null;
