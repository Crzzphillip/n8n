...
type AssistantStore = ReturnType<typeof createAssistantStore>;
let singleton: AssistantStore | undefined;
	...
	function showCodeUpdateToastIfNeeded(errorNodeName: string) {
		if (errorNodeName !== ndvStore.activeNodeName) {
			useToast().showMessage({
				...
				message: `${locale.baseText('aiAssistant.codeUpdated.message.title')}: ${errorNodeName}`,
				duration: 4000,
			});
		}
	}
...
export const useAssistantStore = () => {
	if (!singleton) singleton = createAssistantStore();
	return singleton;
};
