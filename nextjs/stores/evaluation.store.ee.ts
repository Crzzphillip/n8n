...
export const useEvaluationStore = (() => {
	...
	function createEvalStore() {
		...
		const evaluationTriggerExists = computed(() => {
			return workflowsStore.workflow.nodes.some(
				(node: any) => node.type === EVALUATION_TRIGGER_NODE_TYPE,
			);
		});

		function evaluationNodeExist(operation: string) {
			return workflowsStore.workflow.nodes.some((node: any) => {
	...
	return () => {
		if (!singleton) singleton = createEvalStore();
		return singleton;
	};
})();
