...
let singleton: ExecutionsStore | undefined;
...
function createExecutionsStore() {
	...
	const executions = computed<ExecutionSummaryWithScopes[]>(() => {
		const data = Object.values(executionsById.value) as ExecutionSummaryWithScopes[];

		data.sort((a: ExecutionSummary, b: ExecutionSummary) => {
	...
	const executionsByWorkflowId = computed(() =>
		executions.value.reduce<Record<string, ExecutionSummary[]>>((acc, execution: ExecutionSummary) => {
	...
	const currentExecutions = computed<ExecutionSummaryWithScopes[]>(() => {
		const data = Object.values(currentExecutionsById.value) as ExecutionSummaryWithScopes[];

		data.sort(sortFn as (a: unknown, b: unknown) => number);

		return data;
	});

	const currentExecutionsByWorkflowId = computed(() =>
		currentExecutions.value.reduce<Record<string, ExecutionSummary[]>>((acc, execution: ExecutionSummary) => {
			if (!acc[execution.workflowId]) {
				acc[execution.workflowId] = [];
			}
			acc[execution.workflowId].push(execution);
	...
	async function deleteExecutions(sendData: IExecutionDeleteFilter): Promise<void> {
		...
		if (sendData.deleteBefore) {
			...
			allExecutions.value.forEach((execution: ExecutionSummary) => {
...
export const useExecutionsStore = () => {
	if (!singleton) singleton = createExecutionsStore();
	return singleton;
};
