...
interface NDVStoreState {
  ...
  mappingTelemetry: Record<string, string | number | boolean>;
  ...
  setOutputRunIndex: (run?: number) => void;
  ...
  draggableStartDragging: (params: { type: string; data: string; dimensions: DOMRect | null }) => void;
  draggableStopDragging: () => void;
  setDraggableTarget: (target: NDVState['draggable']['activeTarget']) => void;
  setMappingTelemetry: (telemetry: Record<string, string | number | boolean>) => void;
  resetMappingTelemetry: () => void;
  setHoveringItem: (item: TargetItem | null) => void;
  setNDVBranchIndex: (e: { pane: NodePanelType; branchIndex: number }) => void;
  setNDVPanelDataIsEmpty: (params: { panel: NodePanelType; isEmpty: boolean }) => void;
  setMappingOnboarded: () => void;
  setTableHoverOnboarded: () => void;
  setAutocompleteOnboarded: () => void;
  setHighlightDraggables: (highlight: boolean) => void;
  updateNodeParameterIssues: (issues: INodeIssues) => void;
  setFocusedInputPath: (path: string) => void;
}

const getInitialState = (): Omit<NDVStoreState, keyof {
  // Exclude methods from the initial state
  [K in keyof NDVStoreState]: NDVStoreState[K] extends Function ? never : K;
}[keyof NDVStoreState]> => ({
  activeNodeName: null,
  mainPanelDimensions: {
  ...
  isMappingOnboarded: typeof window !== 'undefined' ? localStorage.getItem(LOCAL_STORAGE_MAPPING_IS_ONBOARDED) === 'true' : false,
  isTableHoverOnboarded: typeof window !== 'undefined' ? localStorage.getItem(LOCAL_STORAGE_TABLE_HOVER_IS_ONBOARDED) === 'true' : false,
  isAutocompleteOnboarded: typeof window !== 'undefined' ? localStorage.getItem(LOCAL_STORAGE_AUTOCOMPLETE_IS_ONBOARDED) === 'true' : false,
