...
function createCloudPlanStore() {
    ...
    const codingSkill = computed(() => {
        ...
        if (!('which_of_these_do_you_feel_comfortable_doing' in information) ||
    ...
    const getUserCloudAccount = async () => {
        ...
        try {
            const cloudUser = await cloudApi.getCloudUserInfo(rootStore.restApiContext);
    ...
    const getOwnerCurrentPlan = async () => {
        ...
        try {
            const plan = await getCurrentPlan(rootStore.restApiContext);
            state.data = plan;
...
type CloudPlanStore = ReturnType<typeof createCloudPlanStore>;
let singleton: CloudPlanStore | undefined;

export const useCloudPlanStore = () => {
    if (!singleton) singleton = createCloudPlanStore();
    return singleton;
};
