...
function computed<T>(getter: () => T): { readonly value: T } {
	return { get value() { return getter(); } } as const;
}
...
type InternalCredListener = (action: string, payload: unknown[]) => void;
const actionListeners = new Set<InternalCredListener>();
...
export const useCredentialsStore = (() => {
  ...
  function createStore() {
	...
	const allUsableCredentialsForNode = computed(() => {
		return (node: INodeUi): ICredentialsResponse[] => {
			...
			if (nodeType?.credentials) {
				nodeType.credentials.forEach((cred: INodeCredentialDescription) => {
	...
	const createNewCredential = async (
		...
		actionListeners.forEach((l) => l('createNewCredential', [credential]));
		return credential;
	};
	...
	const updateCredential = async (params: {
		...
		actionListeners.forEach((l) => l('updateCredential', [credential]));
		return credential;
	};
	...
	const deleteCredential = async ({ id }: { id: string }) => {
		...
		actionListeners.forEach((l) => l('deleteCredential', [{ id }]));
	};
	...
	return {
		...
		claimFreeAiCredits,
		$onAction(cb: (result: { name: string; after: (fn: () => void) => void; args: unknown[] }) => void) {
			const listener: InternalCredListener = (action, args) => {
				cb({ name: action, after: (fn) => fn(), args });
			};
...
export const listenForCredentialChanges = (opts: {
	...
	return store.$onAction((result: { name: string; after: (fn: () => void) => void; args: unknown[] }) => {
		const { name, after, args } = result;
		after((returnValue?: unknown) => {
			...
			switch (name) {
				...
				case 'deleteCredential':
					const firstArg = args[0] as { id: string };
					const credentialId = firstArg.id;
