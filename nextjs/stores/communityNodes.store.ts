...
type CommunityNodesStore = ReturnType<typeof createCommunityNodesStore>;
let singleton: CommunityNodesStore | undefined;

function createCommunityNodesStore() {
	...
	const getInstalledPackages = computed(() => {
		return (Object.values(installedPackages.value) as PublicInstalledPackage[]).sort((a, b) =>
	...
	const getInstalledPackage = async (packageName: string) => {
		...
		return getInstalledPackages.value.find((p: PublicInstalledPackage) => p.packageName === packageName);
	};
...
export const useCommunityNodesStore = () => {
	if (!singleton) singleton = createCommunityNodesStore();
	return singleton;
};
