...
export function useCanvasOperations() {
	...
	const updateNodePosition = useCallback((id: string, position: { x: number; y: number }, options?: { trackHistory?: boolean }) => {
	...
	const updateNodesPosition = useCallback((events: CanvasNodeMoveEvent[], options?: { trackHistory?: boolean }) => {
		events.forEach(event => {
			updateNodePosition(event.id, event.position, options);
		});
	}, [updateNodePosition]);

	const deleteNode = useCallback((id: string, options?: { trackHistory?: boolean }) => {
		const node = workflowStore.getState().workflow.nodes.find(n => n.id === id);
		if (!node) return;
	...
	const addNodes = useCallback(async (nodes: Array<{ type: string; position?: XYPosition; parameters?: Record<string, any> }>, options?: {
		dragAndDrop?: boolean;
		position?: XYPosition;
	...
	const createConnection = useCallback((connection: { source: string; target: string; sourceHandle?: string; targetHandle?: string }, options?: { trackHistory?: boolean }) => {
		const sourceNode = workflowStore.getState().workflow.nodes.find(n => n.id === connection.source);
		const targetNode = workflowStore.getState().workflow.nodes.find(n => n.id === connection.target);
	...
	const deleteConnection = useCallback((connection: { source: string; target: string }, options?: { trackHistory?: boolean }) => {
		const sourceNode = workflowStore.getState().workflow.nodes.find(n => n.id === connection.source);
		const targetNode = workflowStore.getState().workflow.nodes.find(n => n.id === connection.target);
	...
	const renameNode = useCallback(async (currentName: string, newName: string, options?: { trackHistory?: boolean }) => {
	...
	const setNodeParameters = useCallback((id: string, parameters: Record<string, unknown>) => {
		const node = workflowStore.getState().workflow.nodes.find(n => n.id === id);
		if (!node) return;
		...
		const command = new ReplaceNodeParametersCommand(id, currentParameters, newParameters, Date.now());
		historyStore.getState().pushCommandToUndo(command);
	}, [workflowStore, historyStore]);

	const toggleNodesDisabled = useCallback((ids: string[]) => {
		ids.forEach(id => {
			const node = workflowStore.getState().workflow.nodes.find(n => n.id === id);
			if (!node) return;
	...
	const setNodeActive = useCallback((id: string) => {
		const node = workflowStore.getState().workflow.nodes.find(n => n.id === id);
		if (node) {
	...
	const setNodeSelected = useCallback((id?: string) => {
	...
	const runWorkflow = useCallback(async (workflowId: string, options?: { destinationNode?: string }) => {
		try {
	...
	const stopCurrentExecution = useCallback(async (executionId: string) => {
		try {
	...
	const tidyUp = useCallback((event: { source: string; nodeIdsFilter?: string[] }) => {
