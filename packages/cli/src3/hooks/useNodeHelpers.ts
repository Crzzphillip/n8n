...
export function useNodeHelpers() {
	...
	const getNodeType = useCallback((type: string, version?: number) => {
		return nodeTypesStore.getState().getNodeType(type, version);
	}, [nodeTypesStore]);

	const isTriggerNode = useCallback((type: string) => {
		return nodeTypesStore.getState().isTriggerNode(type);
	}, [nodeTypesStore]);

	const getNodeInputs = useCallback((node: INodeUi) => {
		const nodeType = getNodeType(node.type, node.typeVersion);
		return nodeType?.inputs || [];
	}, [getNodeType]);

	const getNodeOutputs = useCallback((node: INodeUi) => {
		const nodeType = getNodeType(node.type, node.typeVersion);
		return nodeType?.outputs || [];
	}, [getNodeType]);

	const getNodeProperties = useCallback((node: INodeUi) => {
		const nodeType = getNodeType(node.type, node.typeVersion);
		return nodeType?.properties || {};
	}, [getNodeType]);

	const getNodeCredentials = useCallback((node: INodeUi) => {
		const nodeType = getNodeType(node.type, node.typeVersion);
		return nodeType?.credentials || [];
	}, [getNodeType]);

	const validateNode = useCallback((node: INodeUi) => {
		const nodeType = getNodeType(node.type, node.typeVersion);
		if (!nodeType) {
