## src2/config/schema.ts

Overview: src2/config/schema.ts is a core component within the sv CLI runtime.
...
Place this file at `src2/config/schema.ts` and use the following source:
...
export const schema = {
	...
	userManagement: {
		jwtSecret: {
			doc: 'Set a specific JWT secret (optional - sv can generate one)', // Generated @ start.ts
			format: String,
			default: '',
		...
		isInstanceOwnerSetUp: {
			// sv loads this setting from DB on startup
	...
	redis: {
		prefix: {
			doc: 'Prefix for all sv related keys',
			format: String,
			default: 'sv',
