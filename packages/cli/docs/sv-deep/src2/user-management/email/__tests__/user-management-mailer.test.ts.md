## src2/user-management/email/__tests__/user-management-mailer.test.ts

Overview: src2/user-management/email/__tests__/user-management-mailer.test.ts is a core component within the sv CLI runtime.
...
Place this file at `src2/user-management/email/__tests__/user-management-mailer.test.ts` and use the following source:
		...
		it('should send workflow share notifications', async () => {
			...
			newShareeIds.forEach((id, index) => {
				expect(nodeMailer.sendMail).toHaveBeenNthCalledWith(index + 1, {
					...
					subject: 'Sharer has shared an sv workflow with you',
				});
		...
		it('should send credentials share notifications', async () => {
			...
			newShareeIds.forEach((id, index) => {
				expect(nodeMailer.sendMail).toHaveBeenNthCalledWith(index + 1, {
					...
					emailRecipients: `${id}@user.com`,
					subject: 'Sharer has shared an sv credential with you',
				});
