## src2/commands/ttwf/generate.ts

Overview: src2/commands/ttwf/generate.ts declares a CLI command (TTWFGenerateCommand). Command: `ttwf:generate`.
...
- Classes: TTWFGenerateCommand
- Functions: _waitForWorkflowGenerated
- Exports: TTWFGenerateCommand

### Recreate

Place this file at `src2/commands/ttwf/generate.ts` and use the following source:
...
@Command({
	...
	examples: [
		'$ sv ttwf:generate --prompt "Create a telegram chatbot that can tell current weather in Berlin" --output result.json',
		'$ sv ttwf:generate --input dataset.jsonl --output results.jsonl',
	],
	flagsSchema,
})
