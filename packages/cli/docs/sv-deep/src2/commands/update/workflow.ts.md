## src2/commands/update/workflow.ts

Overview: src2/commands/update/workflow.ts declares a CLI command (UpdateWorkflowCommand). Command: `update:workflow`.
...
- Classes: UpdateWorkflowCommand
- Exports: UpdateWorkflowCommand

### Recreate

Place this file at `src2/commands/update/workflow.ts` and use the following source:
...
export class UpdateWorkflowCommand extends BaseCommand<z.infer<typeof flagsSchema>> {
	async run() {
		...
		this.logger.info('Activation or deactivation will not take effect if sv is running.');
		this.logger.info('Please restart sv for changes to take effect if sv is currently running.');
	}
