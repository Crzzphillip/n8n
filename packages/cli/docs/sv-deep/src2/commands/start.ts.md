## src2/commands/start.ts

Overview: src2/commands/start.ts declares a CLI command (Start). Command: `start`.
...
Place this file at `src2/commands/start.ts` and use the following source:
...
const flagsSchema = z.object({
	...
	reinstallMissingPackages: z
		.boolean()
		.describe(
			'Attempts to self heal sv if packages with nodes are missing. Might drastically increase startup times.',
		)
		.optional(),
});
...
export class Start extends BaseCommand<z.infer<typeof flagsSchema>> {
	...
	async init() {
		...
		this.logger.info('Initializing sv process');
		if (config.getEnv('executions.mode') === 'queue') {
	...
	async run() {
		...
		if (flags.tunnel) {
			...
			this.log(
				'IMPORTANT! Do not share with anybody as it would give people access to your sv instance!',
			);
		}
		...
		// Allow to open sv editor by pressing "o"
