## src2/deprecation/deprecation.service.ts

Overview: src2/deprecation/deprecation.service.ts is a core component (DeprecationService) within the sv CLI runtime.
...
- Classes: DeprecationService
- Exports: DeprecationService

### Recreate

Place this file at `src2/deprecation/deprecation.service.ts` and use the following source:
...
export class DeprecationService {
	private readonly deprecations: Deprecation[] = [
		...
		},
		{
			...
			message: `sv no longer deregisters webhooks at startup and shutdown. ${SAFE_TO_REMOVE}`,
		},
		{
			envVar: 'N8N_RUNNERS_ENABLED',
			message:
				'Running sv without task runners is deprecated. Task runners will be turned on by default in a future version. Please set `N8N_RUNNERS_ENABLED=true` to enable task runners now and avoid potential issues in the future. Learn more: https://docs.n8n.io/hosting/configuration/task-runners/',
		...
		},
		{
			...
			message: `sv has replaced \`tmpl\` with \`tournament\` as expression evaluator. ${SAFE_TO_REMOVE}`,
		},
		{
		...
		},
		{
			envVar: 'EXECUTIONS_PROCESS',
			message:
				'sv does not support `own` mode since May 2023. Please remove this environment variable to allow sv to start. If you need the isolation and performance gains, please consider queue mode: https://docs.n8n.io/hosting/scaling/queue-mode/',
