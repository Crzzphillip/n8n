## src2/__tests__/command-registry.test.ts

Overview: src2/__tests__/command-registry.test.ts is a core component (TestCommand) within the sv CLI runtime.
...
Place this file at `src2/__tests__/command-registry.test.ts` and use the following source:
...
describe('CommandRegistry', () => {
	...
	it('should execute the specified command', async () => {
		process.argv = ['node', 'sv', 'test-command'];
	...
	it('should parse and apply command flags', async () => {
		process.argv = ['node', 'sv', 'test-command', '--flag1', 'value1', '--flag2', '-s', '123'];
	...
	it('should handle alias flags', async () => {
		process.argv = ['node', 'sv', 'test-command', '--flag1', 'value1', '-s', '123'];
	...
	it('should exit with error when command not found', async () => {
		process.argv = ['node', 'sv', 'non-existent-command'];
	...
	it('should display help when --help flag is used', async () => {
		process.argv = ['node', 'sv', 'test-command', '--help'];
