## src2/services/__tests__/community-packages.service.test.ts

Overview: src2/services/__tests__/community-packages.service.test.ts provides a service encapsulating domain logic and integrations.
...
Place this file at `src2/services/__tests__/community-packages.service.test.ts` and use the following source:
...
describe('CommunityPackagesService', () => {
	...
	describe('checkForMissingPackages', () => {
		...
		test('should handle failed reinstallations and record missing packages', async () => {
			...
			expect(communityPackagesService.installPackage).toHaveBeenCalledWith('package-1', '1.0.0');
			expect(logger.error).toHaveBeenCalledWith('sv was unable to install the missing packages.');
			expect(communityPackagesService.missingPackages).toEqual(['package-1@1.0.0']);
		});
