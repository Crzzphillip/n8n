## src2/command-registry.ts

Overview: src2/command-registry.ts is a core component (CommandRegistry) within the sv CLI runtime.
...
Place this file at `src2/command-registry.ts` and use the following source:
	...
	printCommandUsage(commandEntry: CommandEntry) {
		...
		if (commandEntry.examples?.length) {
			...
			output += commandEntry.examples
				.map((example) => `  $ sv ${commandName}${example ? ` ${example}` : ''}`)
				.join('\n');
			output += '\n';
		}
