...
Overview: src2/load-nodes-and-credentials.ts is a core component (LoadNodesAndCredentials) within the sv CLI runtime.
...
- Classes: LoadNodesAndCredentials
- Functions: for, isFullDescription
- Exports: LoadNodesAndCredentials

### Recreate

Place this file at `src2/load-nodes-and-credentials.ts` and use the following source:
...
export class LoadNodesAndCredentials {
	...
	async init() {
		...
		const basePathsToScan = [
			// In case "sv" package is in same node_modules folder.
			path.join(CLI_DIR, '..'),
			// In case "sv" package is the root and the packages are
