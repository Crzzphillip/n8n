## src2/active-workflow-manager.ts

Overview: src2/active-workflow-manager.ts is a core component (ActiveWorkflowManager) within the sv CLI runtime.
...
- Classes: ActiveWorkflowManager
- Functions: which, which
- Exports: ActiveWorkflowManager

### Recreate

Place this file at `src2/active-workflow-manager.ts` and use the following source:
		...
		for (const webhookData of webhooks) {
			...
			try {
				...
				if (activation === 'init' && error.name === 'QueryFailedError') {
					// sv does not remove the registered webhooks on exit.
